...
import static com.minasmob.passageiro.base.BaseActivity.getNumberFormat;
...
public class ServiceAdapter extends RecyclerView.Adapter<ServiceAdapter.MyViewHolder> {
        ...
        if (estimateFare != null) {
            ...
            if (SharedHelper.getKey(context, "measurementType").equalsIgnoreCase(Constants.MeasurementType.KM)) {
                if (estimateFare.getDistance() > 1 || estimateFare.getDistance() > 1.0) {
                    ...
                    holder.capacidade.setText((String.valueOf(obj.getCapacity() )));
                }
            } else {
                if (estimateFare.getDistance() > 1 || estimateFare.getDistance() > 1.0) {
            ...
            if (estimateFare != null) {
                if (SharedHelper.getKey(context, "measurementType").equalsIgnoreCase(Constants.MeasurementType.KM)) {
                    if (estimateFare.getDistance() > 1 || estimateFare.getDistance() > 1.0)
                        ...
                        holder.price.setText(estimateFare.getDistance() + " " + context.getString(R.string.km));
                      //  holder.capacidade.setText( "4" );
                        holder.capacidade.setText((String.valueOf("  " + obj.getCapacity() )));
                       holder.descricao.setText( String.format( obj.getDescricao() ) );
    ...
    class MyViewHolder extends RecyclerView.ViewHolder {
        ...
        private TextView serviceName, price, estimated_fixed,capacidade,descricao;
